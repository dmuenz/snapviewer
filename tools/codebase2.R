...
intro <- capture.output({
  ...
  cat_glue("
Here is a copy, chunked into NUM_CHUNKS .txt files, of all the 1st-party code \\
for a static web app called SnapViewer.
* The app is a tool to help R package developers easily view their unit \\
testing snapshot files created by testthat.
* The app is hosted on GitHub Pages.
* Specifically the code shown here is for the `{gert::git_branch()}` git branch.

Here is a tree-like view of the {proj} project folder, showing names of all \\
files and folders:
...
Below, in fenced code blocks, are the contents of all the 1st-party code \\
(HTML/CSS/JS) and markdown files.

The code is chunked into NUM_CHUNKS txt files for easier uploading to you. \\
Currently you are reading `OUTNAME1`. \\
After you finish reading this file, read the following additional files to \\
understand the entire codebase:

OUTNAME2TON_LIST

Code in the `vendor` folder, from 3rd-party vendors, is not shown, \\
but the `vendor/README.md` file describes the origin and purpose of this code.")
}) |> paste(collapse = "\n")
...
# get file contents in long strings
...
chunks <- chunk_by_nchar(
  c(intro, file_contents, "\nOK, that's the complete 1st-party codebase."),
  max_char = 50000,
  sep = "\n"
)

out_fnames <- sprintf("codebase_%02d.txt", seq_along(chunks))

num_chunks <- length(chunks)

chunks[-num_chunks] <- paste0(
  chunks[-num_chunks],
  "\n\nEnd of `", out_fnames[-num_chunks], "`. Next read `", out_fnames[-1], "`."
)

chunks[1] <- gsub("NUM_CHUNKS", num_chunks, chunks[1])
chunks[1] <- gsub("OUTNAME1", out_fnames[1], chunks[1])
chunks[1] <- gsub("OUTNAME2TON_LIST",
                  paste0('* `', out_fnames[-1], '`', collapse = '\n'),
                  chunks[1])

cat_glue("There will be {length(chunks)} files with the following lengths:
         {paste(nchar(chunks), collapse = ', ')}")
